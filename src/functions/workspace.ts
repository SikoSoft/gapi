...
export async function workspace(
  ...
  switch (request.method) {
    ...
    case HttpMethod.POST: {
      ...
      const res = await WorkspaceLib.create(userId, body.name, body.listConfigs);
      if (res.isErr()) {
        context.error(res.error);
        return { status: 400 };
      }
    ...
    case HttpMethod.PUT: {
      ...
      const res = await WorkspaceLib.update(userId, request.params.id, body.name, body.listConfigs);
      if (res.isErr()) {
        context.error(res.error);
        return { status: 400 };
      }
